                                      ...
                                      const Teuchos::RCP<Teuchos::ParameterList>& responseList)
{
  ...
  if (is_state_a_parameter["sliding_velocity"])
  {
    // Interpolate sliding velocity
    ev = evalUtils.getPSTUtils().constructDOFInterpolationEvaluator("sliding_velocity");
    fm0.template registerEvaluator<EvalT> (ev);
  }
  else
  {
  ...
  if (!is_state_a_parameter["effective_pressure"])
  {
    ...
    ptr_alpha->setNominalValue(params->sublist("Parameters"),params->sublist("FELIX Effective Pressure Surrogate").get<double>(param_name,-1.0));
